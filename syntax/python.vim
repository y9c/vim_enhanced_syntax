...
if !s:Python2Syntax()
  if s:Enabled("g:python_highlight_type_annotations")
    syn match pythonTypeAnno @:\s*['"]\?\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\%(\.\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*\)*['"]\?@hs=s display contained contains=pythonType,pythonString nextgroup=pythonTypeUnion,pythonTypeArgs
...
if version >= 508 || !exists("did_python_syn_inits")
  ...
  HiLink pythonTypeAnno         Typing
  HiLink pythonTypeUnion        Typing
  HiLink pythonTypeAnnoReturn   Typing
  HiLink pythonTypeArgs         Typing
  HiLink pythonType             Typing

  delcommand HiLink

  " default style for custom highlight group (may be overriden in colors)
  " hi Optional gui=italic cterm=italic guibg=Black ctermbg=Black guifg=Grey ctermfg=Grey
endif
